...
export function TaskImageUpload({ taskId, onUploadComplete }: TaskImageUploadProps) {
  const [isUploading, setIsUploading] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [error, setError] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const generateUploadUrl = useAction(api.taskImages.generateUploadUrl);
  const saveImage = useMutation(api.taskImages.saveImage);
  ...
  }, [selectedFile, taskId, generateUploadUrl, saveImage, onUploadComplete]);
